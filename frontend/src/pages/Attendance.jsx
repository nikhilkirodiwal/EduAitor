        ...
        return (
          <button
            ...
            className={`
              ...
              ${isActive ? cfg.active : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300 hover:text-slate-600'}
            `}
          >
            {cfg.label}
          </button>
...
function StatCard({ label, value, valueClass }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200 flex flex-col items-center justify-center py-3 px-2 sm:px-4">
      <span className={`text-xl sm:text-2xl font-bold tabular-nums leading-none ${valueClass}`}>{value}</span>
      <span className="text-[10px] sm:text-[11px] text-slate-400 font-semibold mt-1 uppercase tracking-wide">{label}</span>
    </div>
  );
}
...
function FieldLabel({ children }) {
  return (
    <label className="block text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
      {children}
    </label>
  );
}
...
const selectCls =
  'w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-700 ' +
...
export default function Attendance() {
  ...
  useEffect(() => {
    if (!selClass || !selSection || !selSubject || !selDate || !students.length) return;
    ...
    (async () => {
      ...
      try {
        ...
        const res = await axios.get(`${API}/attendance/existing`, {
          params: { classId: selClass, sectionId: selSection, subjectId: selSubject, date: selDate },
          withCredentials: true,
        });
        ...
        if (existing.length) {
          ...
          setIsEdit(true);
          toast.info('Existing attendance loaded — you are in edit mode.', { autoClose: 3000 });
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        await axios.put(`${API}/attendance/update`, payload, { withCredentials: true });
        toast.success('Attendance updated successfully!');
      } 
      else {
        // POST /attendance/save  — creates new docs
        const res = await axios.post(`${API}/attendance/save`, payload, { withCredentials: true });
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-medium hidden sm:block">Mark &amp; manage attendance</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {students.length > 0 && (
              <button
                ...
                className="
                  flex items-center gap-1.5 bg-slate-900 hover:bg-slate-700
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5 space-y-4">
        ...
        {students.length > 0 && (
          <div className="grid grid-cols-4 gap-2 sm:gap-3">
            <StatCard label="Total"   value={students.length}     valueClass="text-slate-800" />
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-slate-100 bg-slate-50">
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-slate-800">Students</span>
              {students.length > 0 && (
            ...
            {students.length > 0 && (
              <div className="flex items-center gap-1.5">
                <span className="text-[11px] font-semibold text-slate-400 mr-0.5 hidden sm:inline">All:</span>
                {Object.entries(STATUS).map(([key, cfg]) => (
                  <button
                    key={key}
                    onClick={() => markAll(key)}
          ...
          {!loadingStudents && !students.length && (
            <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
              ...
              <p className="text-sm font-semibold text-slate-600">No students loaded</p>
              <p className="text-xs text-slate-400 mt-1">Select a class and section above to begin</p>
            </div>
          )}
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      <th className="text-left px-5 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest w-14">#</th>
                      <th className="text-left px-5 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">Student</th>
                      <th className="text-left px-5 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest w-28">Status</th>
                      <th className="text-left px-5 py-3 text-[10.5px] font-bold text-slate-400 uppercase tracking-widest">Mark</th>
                    </tr>
                  </thead>
                  <tbody>
                    {students.map((student, idx) => {
                      ...
                      return ( 
                        <tr key={student._id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/70 transition-colors">
                          ...
                          <td className="px-5 py-3.5">
                            ...
                            {student.lastName && <span className="text-slate-500 ml-1">{student.lastName}</span>}
                          </td>
                          <td className="px-5 py-3.5">
                        ...
                        {Object.entries(STATUS).map(([key, s]) => (
                          <button
                            ...
                            className={`
                              ...
                              ${status === key ? s.active : 'bg-white border-slate-200 text-slate-400 hover:border-slate-300'}
                            `}
                          >
                            {s.label}
                          </button>
          ...
          {/* Footer */}
          {!loadingStudents && students.length > 0 && (
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-4 sm:px-5 py-4 border-t border-slate-100 bg-slate-50/70">
              <p className="text-xs text-slate-500">
                <span className="font-semibold text-slate-700">{counts.Present ?? 0}</span> present ·{' '}
                <span className="font-semibold text-slate-700">{counts.Absent  ?? 0}</span> absent ·{' '}
                <span className="font-semibold text-slate-700">{counts.Late    ?? 0}</span> late
                {isEdit && (
