...
function SummaryCard({ records }) {
  ...
  return (
    <div
      className="rounded-2xl border border-slate-100 shadow-sm overflow-hidden mb-4"
      style={{
    ...
    >
      ...
      <div className="grid grid-cols-4 border-t border-slate-100">
        {pills.map(({ label, val, color }) => (
          <div
            key={label}
            className={`flex flex-col items-center py-3 border-r last:border-0 border-slate-100 ${label === "Total" ? "bg-slate-50/60" : ""}`}
          >
...
function DateRow({ record, isLatest }) {
  ...
  return (
    ...
    >
      {/* Date block */}
      <div className="flex flex-col items-center justify-center w-11 h-11 rounded-xl bg-slate-50 border border-slate-100 shrink-0">
...
function EmptyState({ month, year }) {
  return (
    <div className="flex flex-col items-center justify-center py-14 text-center">
      <div className="w-16 h-16 rounded-2xl bg-slate-100 flex items-center justify-center mb-3 text-2xl">
        📅
      </div>
